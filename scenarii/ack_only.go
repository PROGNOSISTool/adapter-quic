	...
	for {
		...
		case i := <-incPackets:
			...
			if p.PNSpace() != qt.PNSpaceNoSpace {
				conn.AckQueue[p.PNSpace()] = append(conn.AckQueue[p.PNSpace()], p.GetHeader().GetPacketNumber())
			}
			if p.ShouldBeAcknowledged() {
				...
				if p.PNSpace() == qt.PNSpaceAppData {
					ackOnlyPackets = append(ackOnlyPackets, packet.GetHeader().GetPacketNumber())
				}
